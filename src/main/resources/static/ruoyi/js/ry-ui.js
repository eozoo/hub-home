    ...
    $.extend({
        ...
        table: {
            ...
            importMarkdown: function(formId, width, height) {
                ...
                top.layer.open({
                    ...
                    shade: 0.3,
                    title: '导入' + table.options.modalName + '数据',
                    content: $('#' + currentId).text(),
                    btn: ['<i class="fa fa-check"></i> 导入', '<i class="fa fa-remove"></i> 取消'],
                    // 弹层外区域关闭
                    shadeClose: true,
                    btn1: function(index, layero){
                        var file = layero.find('#file').val();
                        if (file === '' || (!$.common.endWith(file, '.md'))){
                            $.modal.msgWarning("请选择后markdown文件。");
                            return false;
                        }
                        var index = top.layer.load(2, {shade: false});
                        $.modal.disable();
        ...
        operate: {
            ...
            selectTab: function (id) {
                table.set();
                $.modal.openTab("查看" + table.options.modalName, $.operate.selectUrl(id));
            },
            // 查看
            selectUrl: function (id) {
                var url = "/404.html";
                if ($.common.isNotEmpty(id)) {
                    url = table.options.selectUrl.replace("{id}", id);
                } else {
                    var id = $.common.isEmpty(table.options.uniqueId) ? $.table.selectFirstColumns() : $.table.selectColumns(table.options.uniqueId);
                    if (id.length == 0) {
                        $.modal.alertWarning("请至少选择一条记录");
            ...
            view: function(id){
                ...
                $.modal.popupRight(table.options.modalName + "详情", url);
            },
